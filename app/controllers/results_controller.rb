class ResultsController < ApplicationController
  ...
  def find_check_ownership
    ...
    unless current_user.id == @result.user.id
  ...
  def create
    ...
    begin
      ...
      if !game_thr.alive? or game_thr.join(single_game_limit_sec)
        # IS THIS A SECURITY BREACH?
        game_result = YAML.load(YAML.dump(game_thr[:result]))
        if game_result.nil?
          raise ScriptError, 'Bad game script. Check your game and/or agent source code for Ruby language errors.'
        end
      ...
      @result.saved = [ohno.class.to_s, ': ', improve_err_msg.call(ohno)].join(' ')
    ensure
      respond_to do |format|
        # save match to db
  ...
  def destroy
    #@result = Result.find(params[:id])  # now performed in :find_check_ownership
